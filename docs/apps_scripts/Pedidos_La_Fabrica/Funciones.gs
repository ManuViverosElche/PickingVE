...
function sincronizarTodoA_BigQuery() {
  ...
  hojas.forEach(hoja => {
    ...
    if (["FABRICA", "INDICE", "ORIGINAL"].includes(nombreHoja.toUpperCase())) return;
...
function eliminarHojaPorNombre() {
  ...
  var nombreHojaAEliminar = "260833 - IF PLANTS";  // ← Cambia este valor por el nombre de la hoja que deseas eliminar.
