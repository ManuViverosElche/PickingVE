...
const SPREADSHEET_ID_ABDE = '1xsMr6pY7VhOmUrTXxthYrNTBZ1Ll3CWDw42idB4AqmY';  // ID de la hoja de cálculo en Google Sheets de "Pedidos - Abderrahmane"
...
function sincronizarTodoA_BigQuery() {
  ...
  hojas.forEach(hoja => {
    ...
    if (["EXPLANADA", "BLOQUES", "INDICE", "ORIGINAL"].includes(nombreHoja.toUpperCase())) return;
...
function eliminarHojasNoPermitidas() {
  ...
  var hojasPermitidas = ["EXPLANADA", "BLOQUES", "INDICE", "ORIGINAL"];  // Aquí se listan las hojas que NO serán eliminadas.
...
function eliminarHojaPorNombre() {
  ...
  var nombreHojaAEliminar = "260870 - NV PELCKMANS LOMMEL";  // ← Cambia este valor por el nombre de la hoja que deseas eliminar.
