  ...
  hojas.forEach(function(hoja) {
    ...
    if (nombreHoja !== "Indice" && /^[0-9]/.test(nombreHoja)) {
      ...
      if (hoja.getMaxRows() === ultimaConDatos) {
        ...
        hoja.getRange(nuevaFila, 1, 1, hoja.getMaxColumns()).clear({ contentsOnly: false, formatOnly: true });
...
function aplicarReglasFormatoCondicional(hojaPedido) {
  ...
  var rule = SpreadsheetApp.newConditionalFormatRule()
    ...
    .whenFormulaSatisfied('=Y(RESIDUO(FILA();2)=0; ESNUMERO(INDIRECTO("A"&FILA())))') // 📊 Condición: fila par y número en A
...
function existeMaquinaria(hojaOrigen, maquinaria) {
  ...
  const datos = hojaOrigen.getRange('W13:W' + hojaOrigen.getLastRow()).getValues();
  const check = hojaOrigen.getRange('U13:U' + hojaOrigen.getLastRow()).getValues(); // Acopiado?
  const columnaT = hojaOrigen.getRange('T13:T' + hojaOrigen.getLastRow()).getValues(); // Total de planta acopiada
  const columnaS = hojaOrigen.getRange('S13:S' + hojaOrigen.getLastRow()).getValues(); // Total de planta a acopiar
...
function existePedidoEnHoja(hojaDestino, numPedido) {
  ...
  const datos = hojaDestino.getRange('C1:C' + hojaDestino.getLastRow()).getValues();
...
function SetearHoja(hojaDestino) {
  ...
  var rangoTotal = hojaDestino.getRange(1, 1, hojaDestino.getMaxRows(), hojaDestino.getMaxColumns());
  rangoTotal.setFontFamily('Calibri').setFontSize(9).setVerticalAlignment('middle').setHorizontalAlignment('left');
...
function AgregarLineaDePedido(hojaOrigen, hojaDestino, i, j) {
  ...
  rangoFila.setFontFamily('Calibri')
           .setFontSize(9)
  ...
  if (!celdaH.isChecked()) { // solo crea si no existe
...
function ActualizarPedido(hojaOrigen, hojaDestino, maquinaria) {
  ...
  for (var j = 13; j <= lastRowOrigen; j++) {
    ...
    var claveOrigen = construirClave(
      numLinea,
      referencia,
      talla,
      sector
    ...
    if (mapaDestino.hasOwnProperty(claveOrigen)) {
      ...
      if (cantidadNueva !== cantidadDestino) {
        ...
        if (checkboxMarcado === true) {
          ...
          hojaDestino.getRange(rowDestino, 6)
            .setComment("No actualizada: línea cerrada manualmente por operario.");
      ...
      hojaDestino.getRange('C' + insertRow)
                  .setFontStyle('italic');
      hojaDestino.getRange('E' + insertRow)
                 .setFontStyle('italic')
                 .setFontWeight('bold');
      hojaDestino.getRange('G' + insertRow)
                 .setBackground('#B0B0B0')
                 .setFontWeight('bold');
      hojaDestino.getRange('D' + insertRow + ':H' + insertRow)
                 .setHorizontalAlignment('center');


      // Marca la fila como nueva
      hojaDestino.getRange('C' + insertRow + ':E' + insertRow)
...
function AcopiarPedido(hojaIndice, hojaOrigen, hojaDestino, maquinaria) {
    ...
    if (i != 0) {
        ...
        hojaDestino.getRange('C' + (i + 1) + ':G' + (i + 1))
    ...
    hojaDestino.getRange("D" + (i + 2) + ":G" + (i + 2)).mergeAcross().setHorizontalAlignment('center');
    ...
    hojaDestino.getRange("D" + (i + 3) + ":G" + (i + 3)).mergeAcross().setHorizontalAlignment('center');
    ...
    hojaDestino.getRange('C' + (i + 5)).setValue("DESCRIPCIÓN").setHorizontalAlignment('left');
    ...
    for (var j = 13; j <= ultimo; j++) {
        if (hojaOrigen.getRange('W' + j).getValue() == maquinaria && hojaOrigen.getRange('U' + j).getValue() === false) {
    ...
    rangoPedido.setFontFamily('Calibri')
        .setFontSize(9);
    ...
    hojaDestino.getRange('D' + f + ':G' + i)
        .setHorizontalAlignment('center')
        .setFontColor('black');

    // D(f:i) → Sin negrita ni cursiva
    hojaDestino.getRange('D' + f + ':D' + i)
        .setFontWeight('normal')
        .setFontStyle('normal');

    // E(f:i) → Cursiva y Negrita
    hojaDestino.getRange('E' + f + ':E' + i)
        .setFontStyle('italic')
        .setFontWeight('bold');

    // F(f:i) → Sin negrita ni cursiva
    hojaDestino.getRange('F' + f + ':F' + i)
        .setFontWeight('normal')
        .setFontStyle('normal');

    // G(f:i) → Fondo gris (#B0B0B0), Negrita
    hojaDestino.getRange('G' + f + ':G' + i)
        .setBackground('#B0B0B0')
        .setFontWeight('bold');
...
function verificarYAgregarFilas(hojaDestino, fila, cantidad) {
    ...
    if (fila + cantidad - 1 > totalFilas) {
        ...
        var rangoNuevas = hojaDestino.getRange(totalFilas + 1, 1, filasFaltantes, hojaDestino.getMaxColumns());
        rangoNuevas.clearFormat();

        // Aplicar formato base
        rangoNuevas.setFontFamily('Calibri')
                   .setFontSize(9)
                   .setVerticalAlignment('middle')
                   .setHorizontalAlignment('left');

        // 🔹 Columnas A y B en blanco para las nuevas filas
        hojaDestino.getRange(totalFilas + 1, 1, filasFaltantes, 1).setFontColor('white'); // Columna A
        hojaDestino.getRange(totalFilas + 1, 2, filasFaltantes, 1).setFontColor('white'); // Columna B
    }
}
...
function protegerHojaConCheckBox(hojaDestino, f, i) {
  ...
  // ==========================
  var protecciones = hojaDestino.getProtections(SpreadsheetApp.ProtectionType.SHEET);
  var proteccion;
