...
function crearPedido() {
  ...
  const query = `
    ...
    WHERE 
      ...
      AND p.FINCA_CARGA NOT IN ('BORISA', 'LA FÁBRICA')
    ORDER BY 
      DATE(p.FECHA_CARGA), 
      p.NUMERO_PEDIDO, 
      lp.POSICION_PEDIDO
  ...
  try {
    ...
    const spreadsheet = SpreadsheetApp.openById(SPREADSHEET_ID_RESTO_FINCAS);
    ...
    for (let pedidoId in pedidosMap) {
      ...
      lineas.forEach((linea) => {
        ...
        // --------------------------------------------------
        // 2. SOBRESCRIBIR SI EXISTEN COMANDOS EN DESCRIPCIÓN
        // --------------------------------------------------
        if (linea.FINCA_RELEVADA) {
          fincaArticulo = linea.FINCA_RELEVADA;
        }

        if (linea.SECTOR_RELEVADO) {
          sectorDescripcion = linea.SECTOR_RELEVADO;
        }
        ...
        // --------------------------------------------------
        newSheet.getRange(`E${rowIndex}`).setValue(linea.DESCRIPCION_ARTICULO);

        // --------------------------------------------------
        // 5. MARCADO
        // --------------------------------------------------
        newSheet.getRange(`V${rowIndex}`).setValue(linea.MARCADO);

        // --------------------------------------------------
        // 6. OBSERVACIONES
        // --------------------------------------------------
        // 1. Creamos un array temporal para juntar todo
        let todasLasObs = [];

        // 2. Añadimos los campos adicionales solo si tienen contenido (usando truthy check)
        if (linea.MARCA)            todasLasObs.push("MARCA: " + linea.MARCA);
        if (linea.UBICACION_EXTRA)  todasLasObs.push("UBICACIÓN: " + linea.UBICACION_EXTRA);
        if (linea.ACCION_LOGISTICA) todasLasObs.push("OBS: " + linea.ACCION_LOGISTICA);

        // 3. Escribimos en la celda solo una vez, uniendo todo con saltos de línea
        if (todasLasObs.length > 0) {
          newSheet
            .getRange(`X${rowIndex}`)
            .setValue(todasLasObs.join(" - "));
        }

        // --------------------------------------------------
        // 7. TOTAL DE PLANTA (Columna S)
        // --------------------------------------------------
        let unidadesPendientes = 0;
        if (linea.UNIDADES_PENDIENTES != null) {
          unidadesPendientes = parseInt(linea.UNIDADES_PENDIENTES);
          // Insertamos el número puro para poder operar con él
          newSheet.getRange(`S${rowIndex}`).setValue(unidadesPendientes); 
        }

        // --------------------------------------------------
        // 8. TOTAL DE PLANTA ACOPIADA (Columna T) + CHECKBOX (Columna U)
        // --------------------------------------------------
        if (linea.TOTAL_ACOPIADO != null && linea.TOTAL_ACOPIADO != 0) {
          let totalAcopiado = parseInt(linea.TOTAL_ACOPIADO);
          
          // 1. Insertar el valor en la columna T
          newSheet.getRange(`T${rowIndex}`).setValue(totalAcopiado);

          // 2. Comprobación de integridad: ¿Está el pedido completo?
          // Si lo acopiado es igual o mayor a lo pendiente, marcamos el Checkbox
          if (totalAcopiado >= unidadesPendientes && unidadesPendientes > 0) {
            newSheet.getRange(`U${rowIndex}`).setValue(true);
          }
        }
        ...
        // 9. HUELLA DIGITAL
        // --------------------------------------------------
        newSheet.getRange(`Y${rowIndex}`).setValue(linea.HUELLA_DIGITAL);
        rowIndex++;
      });
      ...
      const enlaceHoja = `https://docs.google.com/spreadsheets/d/${SPREADSHEET_ID_RESTO_FINCAS}/edit#gid=${newSheet.getSheetId()}`;
      enviarNotificacionPedido(pedido, enlaceHoja);

      // 3. FORZAR SINCRONIZACIÓN (Crítico)
      // Esto asegura que Google Sheets "asiente" la nueva hoja antes de seguir
      SpreadsheetApp.flush(); 

      //console.log("Preparado para proteger la hoja: " + nombreHoja);

      try {
        // 4. Llamada a la función
        limpiarYProtegerHojas(newSheet);
        //console.log("La llamada a la función se realizó con éxito.");
      } catch (e) {
        console.error("Error al intentar entrar en la función: " + e.message);
      }
    }
