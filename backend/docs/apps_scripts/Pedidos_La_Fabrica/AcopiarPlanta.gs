...
function AcopiarHojaActivaPorOperarios() {
  ...
  for (let fila = 13; fila <= ultimaFila; fila++) {
    ...
    const fecha = Utilities.formatDate(new Date(fechaPedido), Session.getScriptTimeZone(), 'dd/MM/yyyy');
...
function AcopiarTodosLosPedidos() {
  // Copiar pedidos de Jesus, Don Angel
...
function AcopiarPedidos(nombre, LibroPedidos, maquinaria) {
  ...
  hojas.forEach(function(hoja) {
    ...
    if (hoja.getName() != "FABRICA" && hoja.getName() != "INDICE" && hoja.getName() != "ORIGINAL" && 
