 ...
 */
function onEdit(e) {
  // Protección: si la función se ejecuta manualmente sin evento, termina
  if (!e) return;
  ...
  // ---------- 1️⃣ Casilla-botón W6 ----------
  const CELDA_BOTON = "W6"; // Ubicación fija del botón de acopio
  if (e.range.getA1Notation() === CELDA_BOTON && valor === "TRUE") {
    if (e.range.getA1Notation() === "W6" && e.value === "TRUE") {
      EjecutarAcopioDesdeBoton(); // Llama a función con permisos
      e.range.setValue(false); // Desmarca automáticamente
    }
  }

  // ---------- 2️⃣ Casillas de línea ----------
  // Se procesan únicamente filas que correspondan a líneas de pedido (fila >= 13)
  if (fila >= 13) {
    const valorTotal = hoja.getRange("S" + fila).getValue(); // Valor total de la línea (columna S)

    // Columna 20 → actualiza columna U según si el valor ingresado alcanza el total
    if (columna == 20) {
      hoja.getRange("U" + fila).setValue(valor >= valorTotal ? true : false);
    }

    // Columna 21 → rellena o vacía la columna T según el estado del checkbox
    else if (columna == 21) {
...
function Indice() {
  ...
  if (lastRow > 1) {
    ...
    var checkboxesPrevios = hojaIndice
      .getRange(2, 7, lastRow - 1, 1)
      .getValues(); // Columna G (Checkbox)
  ...
  if (lastRow > 1) {
    hojaIndice
      .getRange(2, 1, lastRow - 1, hojaIndice.getLastColumn())
      .clearContent(); // Borra los contenidos
  ...
  hojas.forEach(function (hoja) {
    ...
    if (
      ["FABRICA", "INDICE", "ORIGINAL"].includes(hoja.getName()) ||
      hoja.isSheetHidden()
    ) {
    ...
    hojaIndice
      .getRange(i, 1)
      .setFormula(
        '=HYPERLINK("#gid=' + hoja.getSheetId() + '";"' + numPedido + '")',
      );
    ...
    hojaIndice
      .getRange(i, 4)
      .setFormula(
        "=SUM('" + hoja.getName() + "'!S13:S" + hoja.getLastRow() + ")",
      );
    ...
    hojaIndice
      .getRange(i, 5)
      .setFormula(
        "=SUM('" + hoja.getName() + "'!T13:T" + hoja.getLastRow() + ")",
      );
...
function ordenarHojasPorPedido() {
  ...
  var hojasFijas = ["FABRICA", "INDICE", "ORIGINAL"]; // Define las hojas fijas que no se moverán
