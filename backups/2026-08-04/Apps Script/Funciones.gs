/**
 * Nombre del archivo "Funciones.gs"
 */
...
const SPREADSHEET_ID_LA_FABRICA =
  "1bAQUKkJnKqq8Y85hZTIw_1Iwxr5xBJLTjX-dKPLZ-kc"; // ID de la hoja de cálculo en Google Sheets "Pedidos La Fábrica"
...
function configurarTemporizador() {
  ScriptApp.newTrigger("crearPedido").timeBased().everyHours(1).create();
}
...
function eliminarHojasNoPermitidas() {
  ...
  var hojasPermitidas = ["FABRICA", "INDICE", "ORIGINAL"]; // Aquí se listan las hojas que NO serán eliminadas.
  ...
  hojas.forEach(function (hoja) {
    ...
    if (!hojasPermitidas.includes(nombre)) {
      // Si la hoja no es una de las permitidas, la elimina.
...
function eliminarHojaPorNombre() {
  ...
  var nombreHojaAEliminar = "260554 - 050AUB"; // ← Cambia este valor por el nombre de la hoja que deseas eliminar.
...
function eliminarFechasPasadas(libro) {
  ...
  for (var i = 0; i < fechas.length; i++) {
    ...
    if (fechaFila < hoy) {
      // Si la fecha es anterior al día de hoy
...
function rellenarIndice(hojaIndice, fecha, finca) {
  ...
  hojaIndice
    .getRange(newRowNumber, 3)
    .setFormula(
      '=SUMAR.SI.CONJUNTO(INDIRECTO("\'" & TEXTO(A' +
        newRowNumber +
        ';"dd/mm/yyyy") & "\'!F:F"); ' +
        'INDIRECTO("\'" & TEXTO(A' +
        newRowNumber +
